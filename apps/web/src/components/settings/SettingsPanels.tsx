...
import { stopPlayback } from "../../hooks/useTtsPlayer";
import {
...
export function useSettingsRestore(onRestored?: () => void) {
  ...
  const restoreDefaults = useCallback(async () => {
    ...
    updateSettings({
      ...
      tts: DEFAULT_UNIFIED_SETTINGS.tts,
    ...
    <SettingsPageContainer>
      ...
      <SettingsSection id="behavior" title="Behavior">
        ...
        <SettingsRow
          ...
          description="Show playback controls on completed assistant messages."
          resetAction={
            settings.tts.enabled !== DEFAULT_UNIFIED_SETTINGS.tts.enabled ? (
              <SettingResetButton
                label="text-to-speech"
                onClick={() => {
                  if (!DEFAULT_UNIFIED_SETTINGS.tts.enabled) stopPlayback();
                  updateSettings({
          ...
          control={
            <Switch
              ...
              onCheckedChange={(checked) => {
                if (!checked) stopPlayback();
                updateSettings({ tts: { ...settings.tts, enabled: Boolean(checked) } });
              }}
              aria-label="Enable assistant message text-to-speech"
            />
          }
        />
        <SettingsRow
          title={
            <span className="inline-flex items-center gap-2">
              {settings.tts.enabled ? (
                ...
                <VolumeXIcon className="size-4 text-muted-foreground" aria-hidden="true" />
              )}
              <span>TTS server URL</span>
            </span>
          }
          description="Kokoro-compatible OpenAI speech endpoint used for assistant playback."
          resetAction={
            settings.tts.serverUrl !== DEFAULT_TTS_SERVER_URL ? (
              <SettingResetButton
                label="TTS server URL"
                onClick={() =>
                  updateSettings({
                    tts: { ...settings.tts, serverUrl: DEFAULT_TTS_SERVER_URL },
                  })
                }
              />
            ) : null
        ...
        <SettingsRow
          title="TTS voice"
          description="Voice identifier passed to the speech server."
          resetAction={
            settings.tts.voice !== DEFAULT_TTS_VOICE ? (
              <SettingResetButton
                label="TTS voice"
                onClick={() =>
                  updateSettings({ tts: { ...settings.tts, voice: DEFAULT_TTS_VOICE } })
                }
              />
            ) : null
          }
