...
import { availableJevModelLanes } from "./jevAutoSelect";
...
export interface ChatComposerProps {
  ...
  jevAutoSelectModel: boolean;
  onJevAutoSelectModelChange: (enabled: boolean) => void;
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const hiddenRestingBlockIds = restingBlockDefs
    ...
    .map((def) => def.id);
  const jevAutoSelectAvailable =
    selectedProvider === "codex" && availableJevModelLanes(selectedProviderModels).length >= 2;
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
                type="button"
                variant={props.jevAutoSelectModel ? "secondary" : "ghost"}
                size={composerControlsInStrip ? "xs" : "sm"}
                aria-pressed={props.jevAutoSelectModel}
                aria-label="Auto-select model with Jev"
                disabled={
                  isConnecting ||
                  environmentUnavailable !== null ||
                  (!jevAutoSelectAvailable && !props.jevAutoSelectModel)
                }
                onClick={() => props.onJevAutoSelectModelChange(!props.jevAutoSelectModel)}
