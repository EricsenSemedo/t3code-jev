...
describe("ClientSettings text-to-speech", () => {
  it("defaults complete local playback settings and accepts partial patches", () => {
    expect(decodeClientSettings({}).tts).toEqual({
      enabled: true,
      serverUrl: "http://127.0.0.1:8880",
      voice: "af_heart",
    });
    expect(decodeClientSettingsPatch({ tts: { enabled: false } })).toEqual({
      tts: { enabled: false },
    });
  });
});
